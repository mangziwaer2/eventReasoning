...
For interpretability, every retained edge stores the refined keep probability, predicted relation, strength, and candidate source. The graph metadata records the number of reciprocal and cycle candidates removed by the final decoder. A paper can therefore describe the method as: local edge scoring with relation-aware message passing, followed by an auditable maximum-score acyclic decode.

The present model uses structured event and pair features rather than text embeddings. Its transfer ceiling is consequently determined by feature invariance and candidate recall. ERE-only training can transfer to MIRAI for structural cleanup and temporal ordering, but it cannot learn MIRAI-specific event-code semantics or recover an edge that was never proposed. Report candidate recall, edge F1 before/after refinement, and a no-refinement MIRAI ablation before claiming generalization.
