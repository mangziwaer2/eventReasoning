...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--hidden-dim", type=int, default=None, help="Model hidden dimension. Defaults to train_config.json next to model path, then 192.")
    parser.add_argument("--message-steps", type=int, default=None, help="Model message passing steps. Defaults to train_config.json next to model path, then 4.")
    parser.add_argument("--edge-dim", type=int, default=None, help="Model edge feature dimension. Defaults to train_config.json next to model path, then the current dataset schema.")
    parser.add_argument("--dropout", type=float, default=None, help="Model dropout. Defaults to train_config.json next to model path, then 0.0 for inference.")
    ...
    parser.add_argument("--topology-mode", choices=["none", "temporal-dag"], default="none", help="Optional global decode after refinement; none keeps Qwen directions unchanged.")
...
def build_refined_graph(
    ...
    for edge_index, (edge_desc, keep_prob, strength_prediction) in enumerate(zip(
        edge_descriptions,
        keep_probs,
        ...
        if relation_type not in RELATION_TO_ID:
            relation_type = edge.relation_type if edge is not None else "precedes"
    ...
    refined_graph = CoarseCausalGraph(
        ...
        metadata={**coarse_graph.metadata, "refinement_topology_mode": topology_mode},
    )
    normalized_topology_mode = str(topology_mode).strip().lower().replace("_", "-")
    if normalized_topology_mode == "temporal-dag":
        return apply_temporal_dag_topology(refined_graph)
    if normalized_topology_mode != "none":
