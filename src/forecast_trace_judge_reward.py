...
class JudgeAugmentedGRPOReward:
    """GRPO reward adapter that adds a frozen-Qwen process score.

    The judge contribution is deliberately gated when the final answer is
    wrong, preventing fluent but incorrect traces from dominating answer
    supervision. The deterministic reward remains available for audit.
    """
    ...
    def __call__(self, prompts: Any = None, completions: Any = None, **kwargs: Any) -> list[float]:
        ...
        for index, completion in enumerate(batch):
            ...
            base = self.base.score_completion(
                completion,
                gold=context.gold,
            ...
            # score_completion intentionally exposes only numeric fields; parse
            # once more here to provide the judge with the normalized payload.
            ...
            deterministic_trace = float(base.get("trace", 0.0))
            total = answer + gate * deterministic_trace + self.judge_weight * gate * judge_score
            breakdown = {
                **base,
